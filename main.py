...
async def on_ready():
    ...
    for guild in bot.guilds:
        # Entra no primeiro canal de voz disponível
...
async def on_voice_state_update(member, before, after):
    ...
    if before.channel is None and after.channel is not None:
        ...
        # Move para o canal onde a pessoa entrou
        if voice:
            try:
        ...
        print(f"[{hora}] {member.name} entrou. Iniciando sequência de detecção...")

        # Fala os erros por ~5 segundos
        try:
        ...
        # Manda mensagem de detecção no webhook
